...
SimpleBSpline::SimpleBSpline(int width, int height) : OpenGLDemo(width, height), _activecamera(0), _camera(nullptr) {
    ...
    BSpline bspline(_ptsControle, k);
...
void SimpleBSpline::resize(int width, int height){
    OpenGLDemo::resize(width, height);
    _camera->setviewport(glm::vec4(0.f, 0.f, _width, _height));
    _projection = glm::perspective(_camera->zoom(), float(_width) / _height, 0.1f, 100.0f);
}

void SimpleBSpline::draw() {
    OpenGLDemo::draw();
...
void SimpleBSpline::mouseclick(int button, float xpos, float ypos) {
    _button = button;
    _mousex = xpos;
    _mousey = ypos;
...
void SimpleBSpline::mousemove(float xpos, float ypos) {
    _camera->processmousemovement(_button, xpos, ypos, true);
}

void SimpleBSpline::keyboardmove(int key, double time) {
    _camera->processkeyboard(Camera_Movement(key), time);
}

bool SimpleBSpline::keyboard(unsigned char k) {
    switch(k) {
        case 'a' :
            afficherPtsControle++;
